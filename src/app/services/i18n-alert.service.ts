...
export class I18nAlertService {
  ...
  async show(opts?: AlertOptions): Promise<void> {
    if (opts) {
  ...
  private async translateAlertOptions(opts: AlertOptions): Promise<AlertOptions> {
    // TODO: opts.inputs
  ...
  private translateString(key: string): Promise<string> {
    if (key) {
